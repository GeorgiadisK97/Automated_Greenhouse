...
void setup()
{
  ...
  {
    if (hmd.check_data(DHT_THRESHOLD))
    {
      hmd_flag = false;
      pump.close();
    }
...
void loop()
{
  ...
  { // check if "interval" ms has passed since last time the clients were updated
    ...
    // object["temperature_threshold"] = DHT_THRESHOLD;
    // object["humidity_threshold"] = HMD_THRESHOLD;
    ...
    {
      Serial.println(current_temp_data);
      display_temperature(current_temp_data);
      if (dht.check_data(HMD_THRESHOLD)) // IF THE DATA FROM THE SENSOR IS GREATER THAN THE THRESHOLD
      {
        if (current_temp_data != previous_temp_data && temp_flag) // EXECUTE ONLY IF THE DATA IS DIFFERENT FROM THE PREVIOUS
        {
          temp_flag = false;
          window.open();
          fan.open();
      ...
      {
        if (current_temp_data != previous_temp_data && !temp_flag) // EXECUTE ONLY IF THE DATA IS DIFFERENET FROM THE PREVIOUS
        {
          temp_flag = true;
          window.close();
          fan.close();
...
{ // the parameters of this callback function are always the same -> num: id of the client who send the event, type: type of message, payload: actual data sent and length: length of payload
  ...
  case WStype_TEXT: // if a client has sent data, then type == WStype_TEXT
    ...
    else
    {
      ...
      Serial.println("DHT_THRESHOLD: " + String(DHT_THRESHOLD));
      Serial.println("HMD_THRESHOLD: " + String(HMD_THRESHOLD));
    }
    Serial.println("");
    break;
  }
